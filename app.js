...
function drawBacteria(pos, colorVector, sizeDegree) {
    // Creating new buffers for each bateria
    ...
    for (let i = 0; i < (360 / rotationAngle); i++) {
        ...
        vAry.push(pos);
        vAry.push(newV);
        vAry.push(rotatedVector);

        // normals are vectors

        nAry.push(pos[0], pos[1], pos[2]);
        nAry.push(newV[0], newV[1], newV[2]);
        nAry.push(rotatedVector[0], rotatedVector[1], rotatedVector[2]);

        cAry.push(colorVector[0], colorVector[1], colorVector[2], colorVector[3]);
        cAry.push(colorVector[0], colorVector[1], colorVector[2], colorVector[3]);
        cAry.push(colorVector[0], colorVector[1], colorVector[2], colorVector[3]);
    ...
    console.log("nAry passed");
    gl.bindBuffer(gl.ARRAY_BUFFER, nBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(nAry), gl.STATIC_DRAW);
    var vNormal = gl.getAttribLocation(program, "vNormal");
    gl.vertexAttribPointer(vNormal, 3, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vNormal);

    console.log("vAry passed");
    gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(vAry), gl.STATIC_DRAW);
    var vPosition = gl.getAttribLocation(program, "vPosition");
    gl.vertexAttribPointer(vPosition, 4, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vPosition);

    console.log("cAry passed");
    gl.bindBuffer(gl.ARRAY_BUFFER, cBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(flatten(cAry)), gl.STATIC_DRAW);
    var vColor = gl.getAttribLocation(program, "vColour");
    gl.vertexAttribPointer(vColor, 4, gl.FLOAT, false, 4 * Float32Array.BYTES_PER_ELEMENT, 0);
    gl.enableVertexAttribArray(vColor);

    for (var i = 0; i < tempIndex; i += 3)
        gl.drawArrays(gl.TRIANGLES, i, 3);

    console.log("drawBacteria finished");
...
var frame = 1;
// threshhold size for the bacteria
var threshhold = 30;
// endgame condition
var gameEnd = false;
...
function render(program) {
    ...
    // create a new bacteria at the given frame
    if (frame % 50 == 0) {
      var tempcolor = randomColor();
      var bid = getId(tempcolor);
        ...
        console.log("random bacteria generated");
        if(bacteriaArray.length <= 10)
        bacteriaArray.push(tempBacteria);

    }


    for (var i = 0; i < bacteriaArray.length; i ++){
        drawBacteria(bacteriaArray[i][0], bacteriaArray[i][1], bacteriaArray[i][2]);
    }

    // grow all the bacteria at the given frame
    if (frame % 10 == 0) {

        for (var i = 0; i < bacteriaArray.length; i++) {
            bacteriaArray[i][2] += 0.3;
        
            if (bacteriaArray[i][2] > threshhold) {
                bacteriaArray.splice(i, 1);
                bacteriaAlive = bacteriaAlive-1;
                lives = lives-1;
                if (lives <= 0){
                    gameEnd = true;
                    alert("You Lost!");
        ...
        document.getElementById('scoreDisplay').innerHTML=Math.round(score);
		document.getElementById('bacteriaAlive').innerHTML=bacteriaAlive;
		document.getElementById('lives').innerHTML=lives;
    }
    /*
    // Draw all the bacteria
    for (var i = 0; i < bacteriaArray.length; i++) {
        drawBacteria(bacteriaArray[i][0], bacteriaArray[i][1], bacteriaArray[i][2]);
    }
    */
    ...
    return function(event) {
      ...
      for (let i = 0; i < bacteriaArray.length; i++){
        if (bacteriaArray[i][4] == id){
          score += Math.round(2/bacteriaArray[i][2]);
          bacteriaArray.splice(i, 1);
          break;
        }
      }
...
function getColor(id) {
    if (id > 2<< (8 * 3)) return vec4(0.0, 0.0, 0.0, 1.0);
    var x = (id >> (8 * 0)) & (255);
    var y = (id >> (8 * 1)) & (255);
    var z = (id >> (8 * 2)) & (255);
    return vec4(x / 255.0, y / 255.0, z / 255.0, 1.0);
  }
